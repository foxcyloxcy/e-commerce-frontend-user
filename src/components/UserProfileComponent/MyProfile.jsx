...
import { Box, Container, List, ListItem, ListItemText, ThemeProvider, Divider, IconButton, Drawer, Grid, Typography, Tooltip, Zoom } from '@mui/material';
...
const MyProfile = () => {
  ...
  const SidebarContent = (
    <List sx={{ display: 'flex', flexDirection: 'column', paddingTop: 0 }}>
      <Divider />
      <Tooltip title="Manage your personal profile" placement="right" TransitionComponent={Zoom}>
        <ListItem button onClick={() => setActiveTab('Profile settings')}>
          <ListItemText primary="Personal Profile" />
        </ListItem>
      </Tooltip>
      <Divider />
      <Tooltip title="Change your password" placement="right" TransitionComponent={Zoom}>
        <ListItem button onClick={() => setActiveTab('Change password')}>
          <ListItemText primary="Change Password" />
        </ListItem>
      </Tooltip>
      <Divider />
      <Tooltip title="View and manage your public profile" placement="right" TransitionComponent={Zoom}>
        <ListItem button onClick={() => setActiveTab('Vendor settings')}>
          <ListItemText primary="Your Public Profile" />
        </ListItem>
      </Tooltip>
      <Divider />
      <Tooltip title="Check your pending bids" placement="right" TransitionComponent={Zoom}>
        <ListItem button onClick={() => setActiveTab('My offers')}>
          <ListItemText primary="My Pending Bids" />
        </ListItem>
      </Tooltip>
      <Divider />
      <Tooltip title="View bids you have received" placement="right" TransitionComponent={Zoom}>
        <ListItem button onClick={() => setActiveTab('Offers to me')}>
          <ListItemText primary="Bids Received" />
        </ListItem>
      </Tooltip>
      <Divider />
      <Tooltip title="View items you have listed" placement="right" TransitionComponent={Zoom}>
    ...
    <ThemeProvider theme={ModTheme}>
      <Container sx={{ marginTop: 10, marginBottom: 5, minHeight: '70vh' }}>
        <Grid container spacing={2}>
          ...
          >
            <Typography variant="h6" sx={{ padding: 2, backgroundColor: 'primary.dark', width: '100%' }}>
              My Profile
            </Typography>
            {SidebarContent}
          </Drawer>
          ...
          <Grid item xs={12} md={10}>
            <Content key={activeTab}>{renderContent()}</Content>
          </Grid>
        </Grid>
      </Container>
    </ThemeProvider>
